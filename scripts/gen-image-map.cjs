...
function main() {
  ...
  } else if (fs.existsSync(cardsDir)) {
    const files = fs.readdirSync(cardsDir).filter(f => /\.(png|jpe?g|svg)$/i.test(f));
    ids = Array.from(new Set(files.map(f => f.replace(/\.[^.]+$/, '')).filter(Boolean)));
  }
