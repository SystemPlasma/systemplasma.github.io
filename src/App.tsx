...
function UnlockModal({
  ...
  return createPortal(
    <div
      ...
      style={{ position: 'fixed', top: 0, right: 0, bottom: 0, left: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'rgba(0,0,0,0.4)' }}
      onClick={onClose}
      onKeyDown={(e) => {
    ...
    >
      <div
        className="bg-white dark:bg-slate-900 text-slate-900 dark:text-slate-100 rounded-2xl shadow-2xl w-full max-w-md p-6 flex flex-col gap-3 min-h-[180px] border border-slate-200 dark:border-white/40 dark:ring-2 dark:ring-white/60 dark:ring-offset-2 dark:ring-offset-slate-900"
        style={{ borderRadius: '1rem', padding: '1.5rem', maxWidth: 480, width: '100%' }}
        onClick={(e) => e.stopPropagation()}
      >
        <h2 className="text-xl font-bold text-center text-slate-900 dark:text-slate-100">Unlock an Aspect</h2>
        {lastUnlocked && (
          <div className="text-sm text-slate-800 dark:text-slate-200 text-center font-medium">
...
function CardRow({
  ...
  return (
    <div className="grid grid-cols-12 items-center gap-2 py-1">
      <div className="col-span-5 font-medium truncate text-center">
        {locked ? (
...
function CardPreviewModal({ card, aspectLabel, onClose }: { card: Card; aspectLabel: string; onClose: () => void }) {
  ...
  return createPortal(
    <div
      ...
      style={{ position: 'fixed', inset: 0 as any, zIndex: 2147483647, display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'rgba(0,0,0,0.6)' }}
    ...
    >
      <div
        className="bg-white dark:bg-slate-900 text-slate-900 dark:text-slate-100 rounded-2xl shadow-2xl w-full max-w-2xl p-4 flex flex-col gap-3 pointer-events-auto border border-slate-200 dark:border-white/40 ring-2 ring-black/10 ring-offset-2 ring-offset-white dark:ring-2 dark:ring-white/60 dark:ring-offset-2 dark:ring-offset-slate-900"
      ...
      >
        <div className="flex items-start justify-between">
          <div>
            ...
            <div className="text-sm text-slate-600 dark:text-slate-300">[{card.type}] · R{card.rank} · {aspectLabel}</div>
          </div>
          <button
            type="button"
          ...
          {!errored && url ? (
            ...
            <div className="text-sm text-slate-700 dark:text-slate-300 text-center">
              ...
              {url ? (
                ...
                <>
                  <br />
                  <span className="text-xs text-slate-400">expected: src/assets/cards/{card.id}.png</span>
                </>
              )}
            </div>
          )}
          ...
          <div className="flex items-center gap-2 justify-self-center mx-8">
            {!overrideAll && (
              <button
                ...
                className="rounded-xl border-2 px-6 py-3 text-2xl bg-indigo-600 text-white hover:bg-indigo-700 shadow-md font-semibold min-w-[200px]"
              >
                Unlock Aspects
              </button>
            )}
